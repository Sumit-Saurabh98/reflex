...
export const useCartStore = create<ICartStore>((set, get) => ({
  ...
  isCouponApplied: false,
   ...
   getMyCoupon: async () => {
		try {
			const response = await reflex.get("/coupons");
   ...
   applyCoupon: async (code) => {
		try {
			const response = await reflex.post("/coupons/validate", { code });
  ...
  getCartItems: async () => {
    try {
      const res = await reflex.get("/cart");
  ...
  clearCart: async () => {
        try {
            await reflex.delete(`/cart/clear-cart`);
  ...
  addToCart: async (product: IProduct) => {
    try {
      await reflex.post("/cart", { productId: product._id });
  ...
  removeFromCart: async (productId) => {
		try {
            await reflex.delete(`/cart`, { data: { productId } });
    ...
    updateQuantity: async (productId, quantity) => {
		...
		await reflex.put(`/cart/${productId}`, { quantity });
		set((prevState) => ({
